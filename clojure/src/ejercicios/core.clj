...
(defn maximo-lista
  ...
  [coll]
  (reduce (fn [max-so-far x] (if (> x max-so-far) x max-so-far)) coll))
...
(defn mi-filter
  ...
  (if (empty? coll)
    ()
    (let [x (first coll) rest-coll (rest coll)]
      (if (pred x)
        (cons x (mi-filter pred rest-coll))
...
(defn contar-con
  "CLJ-16: Cuenta cuántos elementos de coll satisfacen pred.
   ...
   (contar-con any? [])             => 0"
  [pred coll]
  ( reduce (fn [count x] (if (pred x) (inc count) count)) 0 coll))
...
(defn aplanar-profundo
  ...
  (if (empty? coll)
    ...
    (let [x (first coll) rest-coll (rest coll)]
      (if (vector? x)
        (concat (aplanar-profundo x) (aplanar-profundo rest-coll))
        (cons x (aplanar-profundo rest-coll))))))
...
(defn frecuencias-manual
  ...
  [coll]
  (reduce (fn [frecs x]
            (assoc frecs x (inc (get frecs x 0))))
          {}
          coll))
...
(defn agrupar-por-tipo
  ...
  (reduce (fn [acc reg]
            (let [tipo (:tipo reg)]
              (assoc acc tipo (conj (get acc tipo []) reg))))
          {}
          registros))
...
(defn aplicar-descuento
  ...
  [productos]
  (map (fn [producto]
         (assoc producto :precio (* (:precio producto) 0.9)))
       productos))
